...
Decision tree is a machine learning algorithm that is very versatile that is able to perform classification and regression tasks.
When it comes to a decision tree there are important terms that one should know which are: 
...
### Loan Data Feature Selection for Decision Tree

- Lets look at the features of the data set by using the Boruta function and calculating the mean importance.

```{r loanboruta, message=FALSE, warning=FALSE}
set.seed(525)
...
By looking at this we can see that the biggest feature in the data is Credit History with a mean imp that is above 50. Credit_History is definitely an important feature because it allows one to know if one pays for their bills on time, or if they are generally late with their payments. The feature that follow are important but don't have the same mean importance is ApplicantIncome and CoapplicantIncome. Which its understandable why those two features would be important when it comes to loans is because one would want to make sure that there is enough income to pay for the loan.  



### Plot the Decision Trees 

In order to create the decision tree we will use the rpart package to calculate, and rpart.plot package to plot it.

```{r fig.height=10, fig.width=10}
dt_loan_model1 <- rpart(Loan_Status ~., data = train_set, method = 'class')
rpart.plot(dt_loan_model1 )
```

This shows us that 68% of people have a credit history
...
This shows the error rate of the model and it is of the complexity parameter. 

```{r}
print(dt_loan_model1$cptable)
```

Looking at the error one can see that the best complexity parameter is at .016, because the xerror is at its lowest at .584. we will use the cp in the next model to see if there is an improvement in the decision tree and also compare the confusion matrix. The complexity parameter allows one to choose the optimal tree size. 

### Confusion Matrix of first Decision Tree
...
Here you can see there is an accuracy rate of 76%, kappa is at 38%. Looking at the prediction you can see that 6 false postives, and 23 false negatives, while 78 are predicted correctly as Y and 16 as no.


### Decision Tree with Hyperparameters 

For this decision tree model we are going to include the complexity parameter of .016 as a hyperparameter which was obtained from the first model. One might wonder how does using complexity parameter change the decision tree. By looking at the decision tree plot you can see that it is a drastic change. 

```{r}
dt_loan_model2 <- rpart(Loan_Status ~.,
                   method="class",
                   data=train_set,
                   control=rpart.control( cp= 0.016 ),
                   parms=list(split='information'))

rpart.plot(dt_loan_model2)
```
This only showcases Credit_History as the most important feature, which its not surprising since the mean importance was so high that no other feature was even close.  


##### Error Rate of the Model 

```{r}
plotcp(dt_loan_model2)
```

The Error rate of this model is just a straight line. 
...
```{r}
unseen_prediction1 <-predict(dt_loan_model2, test_set, type = 'class')

confusionMatrix(table(unseen_prediction1, test_set$Loan_Status))
```

The accuracy has improved after using the complexity parameter, you can see that the accuracy is at 79%, and Kappa is at 45%. The false positive is only 2, and the false negative is at 23, which didn't change from the original. But it correctly predicted 82. Based on this you can see that the best model is the one with the hyperparameter with the complexity parameter.


### Conditional Parting Plot 

This shows regression relationship of the conditional inference trees estimate that is partitioned by binary recursive in a  framework that is conditional inference. The package used for this is he party package and the partykit package, and the ctree function is used.
...
You can see again that credit history is the most important feature. Here you can see that n=62 is the number without a credit history, and n=326 is the number with a credit history. 
...
Base-learning models: Boosted algorithms almost always use decision trees as the base-learner. We will be using boosting in the context of regression trees.
...
1) Fit a decision tree to the data: F1(x)=yF1(x)=y,

2) We then fit the next decision tree to the residuals of the previous: h1(x)=y−F1(x)h1(x)=y−F1(x),

3) Add this new tree to our algorithm: F2(x)=F1(x)+h1(x)F2(x)=F1(x)+h1(x),

4) Fit the next decision tree to the residuals of F2F2: h2(x)=y−F2(x)h2(x)=y−F2(x),

5) Add this new tree to our algorithm: F3(x)=F2(x)+h1(x)F3(x)=F2(x)+h1(x),
...
local_obs <- test_set[1:2, ]

# apply LIME
explainer <- lime(train_set, gbm.fit.final)
explanation <- explain(local_obs, explainer, n_features = 5)
plot_features(explanation)
```


Using LIME to understand why a prediction resulted in a given value for a single observation. We will select two observations from test_set. The results show the predicted value of Case 2: 1.74 and Case 6: 1.74, the model fit is pretty good and the most influential variables driving the predicted value for each observation, Credit_History. 

```{r}
# predict values for test data
pred <- predict(gbm.fit.final, n.trees = gbm.fit.final$n.trees, test_set)

test_set$Loan_Status<-as.numeric(test_set$Loan_Status)

# results
caret::RMSE(pred, test_set$Loan_Status)
