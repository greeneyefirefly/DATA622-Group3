...
# Make 3 dataframes with just age, race, sex so we can use the table function to 
# see how the datapoints fall into each cluster.
...
Cluster 3 can be considered a youth Cluster with a mean age of 26.07, while Cluster 1 have a mean age of 48.13 and Cluster 2 have a mean age of 41.66. 
...
```{r drug svm plot}
model_svm <- svm(Suicide~ Alcohol+Cocaine , data = drugs_df, kernel = "polynomial", cost=10)

plot(model_svm,drugs_df,
     Alcohol~Cocaine)
```

Looking at the svm plot and seeing how the hyperplane is it seems that it is less likely for someone to use these top feature substances in order to commit suicide. 
...
```{r}
model_svm <- svm(Suicide~  Opioids+Abuse , data = svm_train, kernel = "polynomial", cost=10)

plot(model_svm,svm_train,
     Opioids~Abuse)
```

Looking at the hyperplane and the fact that the points are along zero mark it seems that someone under these top features would not commit suicide. 
