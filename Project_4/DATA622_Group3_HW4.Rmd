...
`Age`: For this dataset, individuals of several specific ages (e.g., 22, 24, 36, and 48) have attempted suicide at rates of approximately 0.60 or higher. The relatively small size of the set, however, suggests that these ages may describe relatively small groups of individuals. 

```{r suicide_age, fig.height=3}
tab <- with(df, table(Age, Suicide))
tab <- as.data.frame(prop.table(tab, margin = 1)) %>%
  filter(Suicide == '1')
...
tab %>%
  ...
  theme(legend.position='none') +
  labs(title = 'Proportion attempting suicide, by age',
       y = 'Proportion') +
