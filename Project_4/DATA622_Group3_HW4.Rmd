...
One of the more popular algorithms for clustering is K-means. It divides the observations into discrete groups based on some distance metric.

1) Excluding Sex, Race, Age because it might be too correlated with group membership.

2) K-means requires specification of the number of clusters. 

#### Data Prep

Initial is used to identify the individual in the study and is excluded as well.
Age, Sex, and Race is excluded because it might be too correlated with group membership.
...
# Make 3 dataframe with just age, race , sex so we can use the table function to see how the datapoints fall into each cluster.
...
The optimal cluster number to use is 3.
...
Using K-means clustering we get 3 clusters of sizes 22, 15, 18.
...
The table function allows how to see how many of the data points from either age, sex , race lands into one of the 3 clusters.
...
Using the Hartigan's rule, 3 is the optimal number for the clusters. Using two different clustering methods, k-means and Hierarchical clustering to depict the different 3 clusters. Hierarchical clustering depicts the clusters with a dendrogram. Using the table function to show which age group, sex group , or race group belongs to which of the three cluster groups. 
...
PCA only works with non-null values, so any records that include missing values is dropped. Additionally, since the data dictionary indicates that ADHD questions should be on scale of 1-4, records in the dataset labeled '5' are eliminated, as this is likely a data-entry error. 
...
Next, using a 90% coutoff, the total number of principal components that the data can be reduce to is determined. In other words, the number of components that make up 90% of the variability in the data is accepted. This threshold is identified by looking at a cumulative plot of the variances. 
...
Once again, take a look at these components with respect to the `Suicide` variable. There is still quite a bit of overlap, nonetheless there is some distinctions between the two classes (lower PC1 is more indicative of non-Suicide individuals).
