...
K-means clustering consists of defining clusters so that the total intra-cluster variation is minimized. For clustering distance measures, the classification of variables into groups requires some methods to compute the distance between each pair of variables. One method used is the distance matrix. 

1. Specify the number of clusters (K) to be used.
...
5. Iteratively minimize the total within sum of square. Iterate steps 3 and 4 until the cluster assignments stops or the maximum number of iterations is reached.

#### Data Preparation

To perform a cluster analysis in R, the data should be prepared:

Rows are observations and cloumns are variables.

Any missing value in the data must be omitted.

The data must be scale to make variables comparable. Variables must be transformed so they have mean zero and standard deviation of one.

```{r}
ADHD_df <- read_csv("https://raw.githubusercontent.com/greeneyefirefly/DATA622-Group3/main/Project_4/ADHD_data.csv")
...
Create and visualize the distance matrix of the variables.
...
Using scatter plot to compared Suicide Attempts vs Age. There doesn't seem to be any relationship with age and suicide attempts. However there seem to be more suicide attempts by patients in cluster 2.
...
The optimal cluster chosen appears to be 3.
...
Observe the means of each variable of each of the three clusters.
...
Using the variable description of each column, predictions can be made for each clusters.
...
Cluster 1 and Cluster 3 are overlapping and quite similar to one another. Cluster 2 is alone by itself. This can be seen in the data from the table above. Cluster 1 and 3 have patients answering on average higher than cluster 2 for ADHD Questions.
Cluster 3 can be considered a youth cluster with a mean age of 26.07, while cluster 1 have 48.13 and cluster have 41.66. 

For the mood questions, there aren't much difference between each clusters. However for mood question 3, Cluster 1 and 3 are both 2.4 while Cluster 2 is 1.8.

Everyone in cluster 2 has done cocaine. Only cluster 1 have patients doing sedatives.hypnotics. 

Cluster 3 patients have 4 times more likely to have a court order compared to cluster 1 and 2.

Education is all around Grade 12.5 - 12.6 for all three clusters.


Cluster 3 seems to have almost twice the amount of suicide attempts compared to cluster 1 and 2.
...
Only two are considered to be important, which are 'Opioids' and 'Alcohol'.
...
```{r boruta_all, message=FALSE, warning=FALSE}
set.seed(525)
...
The most important was 'Abuse', and the others are somewhat as important. 
...
p1 = ggplot(svm_test1, aes(x = Suicide, y = ADHD_pred, color = Suicide)) +
  geom_jitter(size = 3) +
  scale_color_manual(values = pCol)+
  labs(title = 'svmLinear Accuracy : 0.71')
```
...
* Kuhn, M., and Johnson, K. (2013). [*The basics of encoding categorical data for predictive models*](!http://appliedpredictivemodeling.com/blog/2013/10/23/the-basics-of-encoding-categorical-data-for-predictive-models). Applied Predictive Modeling. Accessed April 21, 2021.
* [*Clustering in R*](!blog.dominodatalab.com/clustering-in-r/). Data Science Blog by Domino, 31 Mar. 2021. 
* Alboukadel, et al. [*5 Amazing Types of Clustering Methods You Should Know.*](!www.datanovia.com/en/blog/types-of-clustering-methods-overview-and-quick-start-r-code/) Datanovia, 25 Dec. 2019. 
