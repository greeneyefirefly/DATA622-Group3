...
Principal Component Analysis (PCA) is a method that allows us to summarize information in datasets that contain correlated numerical variables. Each feature is considered a dimension, and the goal of PCA is to reduce dimensionality while preserving the information contained in the original dataset. The resulting features (called principal components) are linear combinations of the original features and contain the maximum variation (information) in the dataset. 

The value of PCA is that it allows us to: (1) identify correlated variables and outliers, (2) discover hidden patterns and trends and (3) remove redundancy and noise. 

Traditional PCA works with continuous, numerical features and creates a Pearson correlation matrix behind-the-scenes. Pearson correlations assume that all variables are normally distributed -- in other words, that they are continuous, quantitative, symmetric, and bell shaped. Although we can technically apply PCA to the raw data in our dataset and get an output, since most of the features are dichotomous (ie, binary) or ordinal, the output will not hold much meaning. 

The alternative to this is to base PCA off of a polychoric correlation. These correlations assume that the variables are ordered measures of an underlying continuum, so they don't need to be continuous or symmetrical (which works out well for our binary and ordinal features!). Like a Pearson correlation, the values of a polychoric correlation range from -1 to 1 and the the value/sign measures the strength and direction of the relationship. 

#### PCA - Dataset
Broadly speaking, our base dataset contains 3 main categories of features (demographics, questionnaire responses, and drug use) and a few additional miscellaneous features. Off the bat, we'll eliminate the `Psych.meds` variable, because it has more than 50% of its values missing. We will also leave demographic information (`Age`, `Sex`, and `Race`) out of the PCA because we want to preserve these values. 

The approach for this analysis will be to identify the optimal reductions by first performing PCA on the 2 questionnaire sets and drug variables independently and then on a combination of these categories and the remaining miscellaneous categories together. The independent PCA of questionnaire answers & drug variables assumes that the features within these categories are correlated, but that the categories are independent of one another. We will compare the results of these analyses to the final results with all variables included together. 

PCA only works with non-null values, so we will drop any records that include missing values. Additionally, since our data dictionary indicates that ADHD questions should be on scale of 1-4, we will eliminate the record in the dataset that is labeled 5, as this is likely a data-entry error. 
...
The resulting eigenvalues are the variances for each of the components. We can visualize them as percentages in a scree plot. 

```{r pca_adhd_scree}
adhd_var <- adhd_pca$values/sum(adhd_pca$values)
rounded_adhd_var <- round(adhd_var,2)

qplot(c(1:length(adhd_pca$values)), adhd_var) + 
  geom_line() + 
...
We can see that 65% of the variance is explained by principal components 1 and 2 alone! Since our third task is to use an SVM to predict the `Suicide` variable, we can see if there are any obvious patterns between this variable and the first two principal components. To do this, we will use the loadings matrix to calculate the final pca scores for each observation in the dataset. We will append these values to our original dataset and plot with respect to the `Suicide` variable to see if we can identify any meaningful relationships. 

```{r, pca_adhd_final}
adhd_pca$scores <- factor.scores(df_adhd, adhd_pca) 
df_adhd_final <- cbind(pca_base %>% select(Suicide), adhd_pca$scores$scores)

ggplot(df_adhd_final, aes(PC1, PC2, col = Suicide, fill = Suicide)) +
  stat_ellipse(geom = 'polygon', col = 'black', alpha = 0.5) +
  geom_point(shape = 21, col = 'black')

```

We can see that there is still quite a bit of overlap between the two classes. To determine the total number of principal components that we can reduce to, we will use 90% as a cutoff. In other words, we will accept the number of components that make up 90% of the variability in the data. We can identify this threshold by looking at a cumulative plot of the variances. 
...
Our final analysis shows that we can reduce the number of ADHD features to 9 principal components and retain 90% of the variability in the data. 


#### Mood Disorder Questions 
We will take a similar approach with the Mood Disorder questions. First we'll calculate the polychoric correlation matrix and pass it to the principal function. 
...
Next, we can visualize the eigenvalues as percentages in a scree plot. 

```{r pca_mood_scree}
mood_var <- mood_pca$values/sum(mood_pca$values)
rounded_mood_var <- round(mood_var,2)

qplot(c(1:length(mood_pca$values)), mood_var) + 
  geom_line() + 
...
We can see that 66% of the variance is explained by the first two principal components. We'll take a look at these components with respect to the `Suicide` variable. 

```{r, pca_mood_final}
mood_pca$scores <- factor.scores(df_mood,mood_pca) 
df_mood_final <- cbind(pca_base %>% select(Suicide), mood_pca$scores$scores)

ggplot(df_mood_final, aes(PC1, PC2, col = Suicide, fill = Suicide)) +
  stat_ellipse(geom = 'polygon', col = 'black', alpha = 0.5) +
  geom_point(shape = 21, col = 'black')

```
Still quite a bit of overlap, but we can start to see some distinctions between the two classes (lower PC1 is more indicative of non-Suicide individuals).

Finally, we can identify the number of principal components to use in order to meet the 90% variability cutoff. 
...
Our final analysis shows that we can reduce the number of Mood Disorder features to 7 principal components and retain 90% of the variability in the data. 

#### Drug Features

Finally, we'll take a look at the drug features. This subset of data will include the following features: `Alcohol`, `THC`, `Cocaine`, `Stimulants`, `Sedative.hypnotics`, `Opioids`, and `Subst.Dx`.
```{r pca_drug}
df_drug <- pca_base %>% 
...
Once again, we can visualize the eigenvalues as percentages in a scree plot. 

```{r pca_drug_scree}
drug_var <- drug_pca$values/sum(drug_pca$values)
rounded_drug_var <- round(drug_var,2)

qplot(c(1:length(drug_pca$values)), drug_var) + 
  geom_line() + 
...
To meet the 90% threshold, we will need to use 5 principal components. Since there are only 7 unique drug features, this reduction might not be as worth it to pursue. 

#### All Numeric Features
Finally, we'll take a look at all of the features combined. This approach assumes dependence of features across categories (questionnaires, drugs, and miscellaneous). 
...
The final scree plot will show us the variances for all of the principal components. 

```{r pca_all_scree}
all_var <- all_pca$values/sum(all_pca$values)
rounded_all_var <- round(all_var,1)

qplot(c(1:length(all_pca$values)), all_var) + 
  geom_line() + 
...
Only about 40% of the variability is explained by the first two components. 

```{r, pca_all_final}
all_pca$scores <- factor.scores(df_all, all_pca) 
df_all_final <- cbind(pca_base %>% select(Suicide), all_pca$scores$scores)

ggplot(df_all_final, aes(PC1, PC2, col = Suicide, fill = Suicide)) +
  stat_ellipse(geom = 'polygon', col = 'black', alpha = 0.5) +
  geom_point(shape = 21, col = 'black')

```

Despite this, we can see from the plot of the two components versus the `Suicide` variable that the lower values of PC1 are associated with less suicides. 

One final cumulative scree plot will help us to identify how many principal components will need to be kept to meet the 90% threshold. 
...
To retain 90% of the variance in the dataset, we would need to keep 17 principal components. 


#### PCA Conclusions

We explored the differences between completing PCA independently on the 3 categories of features (ADHD questionnaire, MD questionnaire, and drug features) and across all cateogories. We found: 

* The ADHD question PCA analysis resulted in the first principal component accounting for almost 60% of the variance in the data. To retain 90% of the variance, we would need to keep 9 of the principal components. This results in a reduction of ADHD features by 50%. 
* The Mood disorder question PCA analysis resulted in the first principal component accounting for 53% of the variance in the data. When visualizing this against the `Suicide` feature, we started to see some separation of the 2 classes based on the first two components alone. To retain 90% of the variance, we would need to keep 7 of the principal components. This results in a reduction of Mood Disorder Features by a little over 50%. 
* The drug feature PCA analysis resulted in the first principal component accounting for about 34% of the variance in the data. To retain 90% of the variance, we would need to keep 5 of the principal components. Since there are only 7 drug features in the dataset, this reduction might not be necessary. 
* The final PCA analysis with all features resulted in the first principal component accounting for 30% of the variance in the data. To retain 90% of the variance, we would need to keep 17 principal components. 
